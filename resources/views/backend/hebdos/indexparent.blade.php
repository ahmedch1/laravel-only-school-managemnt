...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold"> Les Programmes Hebdomadaires</h2>
            </div>
        </div>
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            ...
            @foreach ($hebdos as $hebdo)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div
                        class="w-2/12 px-3 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $hebdo->name }}</div>
                    <div
                        class="w-2/12 px-3 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $hebdo->link }}</div>
                    <div
                        class="w-2/12 px-3 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $hebdo->description }}</div>

                    <div class="w-2/12 flex items-center justify-end px-3">
                        <a href="{{ route('hebdos.show',$hebdo->id) }}"
...
@push('scripts')
    <script>
        $(function () {
            $(".deletehebdo").on("click", function (event) {
