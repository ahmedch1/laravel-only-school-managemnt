        ...
        @foreach ($contacts as $key => $contact)
            <div
                ...
                <div class="text-gray-700 font-bold">
                    <div class="mb-6">
                        <div class="text-lg uppercase">{{ $contact->name }}</div>
                        <div class="text-sm lowercase leading-tight">{{ $contact->email }}</div>
                    </div>

                    <div class="flex items-center justify-center">
                        <div class=" text-sm text-left">Message :</div>
...
<div class="w-full block mt-4 sm:mt-8">
    <div class="flow-root">
        @foreach ($pcontacts as $key => $pcontact)
            <div
                class="w-full bg-gray-200 text-center border border-gray-300 rounded px-8 py-6 mb-4 {{ ($key>=1) ? 'ml-0 sm:ml-2' : '' }}">
                <div class="text-gray-700 font-bold">
                    <div class="mb-6">
                        <div class="text-lg uppercase">{{ $pcontact->user->name }}</div>
                        <div class="text-sm lowercase leading-tight">{{ $pcontact->user->email }}</div>
                    </div>

                    <div class="flex items-center justify-center">
                        <div class=" text-left">{{ $pcontact->subject }}</div>
                    </div>

                    <div class="flex items-center justify-center mt-5">
                        <div class=" text-left">Message :</div>
                        <div class=" text-sm text-left ml-2">{{ $pcontact->message }}</div>
                    </div>
