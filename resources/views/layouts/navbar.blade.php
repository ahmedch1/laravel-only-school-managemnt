<div
    ...
    <div class="flex items-center text-white">
        ...
        <span class="font-semibold text-sm sm:text-xl tracking-tight">Ecole Tunisie</span>
    </div>
    <div class="relative">
        @auth
                ...
                <div class="hidden h-24 w-48" id="navdropdown">
                    <div class="px-8 py-4 border-t border-blue-800">
                        <a href="{{ route('profile') }}"
                           class="flex items-center pb-3 text-sm text-gray-200 font-semibold">
                            <svg class="h-4 w-4 mr-2 fill-current text-gray-200" aria-hidden="true" focusable="false"
                                 data-prefix="fas" data-icon="user-alt" class="svg-inline--fa fa-user-alt fa-w-16"
                        ...
                        <form action="{{ route('logout') }}" method="POST" class="pb-2">
                            ...
                            <button class="flex items-center text-sm text-gray-200 font-semibold focus:outline-none"
                                    type="submit">
                                <svg class="h-4 w-4 mr-2 fill-current text-gray-200" aria-hidden="true"
                                     focusable="false" data-prefix="fas" data-icon="sign-out-alt"
        ...
        @else
            <div class="flex items-center">
                @if (Route::has('login'))
                    <div>
                        <a class="flex items-center mr-4 text-sm text-gray-200 font-semibold"
                           href="{{ route('login') }}">
